...
| 5 | No allergy cross-referencing at all | 9 | 3 | 10 | 270 | **Partially mitigated (v0.9)** | Penicillin-class allergy checking added for amoxicillin specifically - the single clearest, highest-value case, not a general allergy system. Every other drug in the rulebook still has zero allergy awareness. Severity/occurrence/detection unchanged for the general case; this row will be re-scored down once broader coverage exists. |

## Medium priority (RPN 100-200)

| # | Failure mode | S | O | D | RPN | Status | Notes |
|---|---|---|---|---|---|---|---|
| 6 | No drug-drug interaction checking | 6 | 3 | 9 | 162 | **Partially mitigated (v0.9)** | A narrow opioid + CNS-depressant flag now exists for fentanyl/oxycodone specifically (benzodiazepines, other opioids, alcohol) - the single highest-stakes interaction case in this rulebook. General interaction checking (e.g. concurrent paracetamol + ibuprofen) remains genuinely out of scope. |
...
| # | Failure mode | S | O | D | RPN | Status | Notes |
|---|---|---|---|---|---|---|---|
| 11 | Route of administration (PO vs IV) not distinguished - same thresholds applied regardless | 5 | 3 | 6 | 90 | **Mitigated (v0.9)** | Route caution now checked for all 7 drugs, with per-drug calibration rather than one blanket rule - paracetamol IV gets a specific formulation-error warning, loratadine flags any non-oral route as a likely transcription error, dexamethasone deliberately does NOT flag IV/IM (clinically interchangeable with oral in practice). Detection score improved accordingly; residual risk is in the specific route-threshold calibration itself, not the absence of the check. |
